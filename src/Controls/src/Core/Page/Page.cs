...
After:
		...
		/// <inheritdoc cref="DisplayActionSheet(string, string, string, FlowDirection, string[])"/>
		public Task<string> DisplayActionSheet(object sender, NotifyCollectionChangedEventArgs e)
*/
Element> InternalChildren { get; } = new ObservableCollection<Element>();
...
After:
		internal void SetTitleView(string title, string message, string cancel)
		{
			return DisplayAlert(title, message, null, cancel, FlowDirection.MatchParent);
*/
string cancel, string destruction, FlowDirection flowDirection, params string[] buttons)
		{
			var args = new ActionSheetArguments(title, cancel, destruction, buttons);
...
After:
		/// <inheritdoc cref="DisplayAlert(string, string, string, string, FlowDirection)"/>
		public Task<NavigatedToEventArgs> NavigatedTo;
*/
