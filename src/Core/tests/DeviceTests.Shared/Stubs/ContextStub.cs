...
/* Unmerged change from project 'Core.DeviceTests.Shared(net8.0-windows10.0.19041)'
Before:
				return _manager ??= _services.GetRequiredService<IAnimationManager>();
#if ANDROID
			if (serviceType == typeof(IFontManager))
...
/* Unmerged change from project 'Core.DeviceTests.Shared(net8.0-windows10.0.20348)'
Before:
				return _manager ??= _services.GetRequiredService<IAnimationManager>();
#if ANDROID
			if (serviceType == typeof(IFontManager))
